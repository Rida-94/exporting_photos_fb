...
- (void)setSelected:(NSArray *)selected {
    ...
    //self.albumVC.selected = [selected sortedArrayUsingSelector:@selector(localizedCaseInsensitiveCompare:)];
}

- (NSArray *)selected {
    return self.albumVC.selected;
    //return [self.albumVC.selected sortedArrayUsingSelector:@selector(localizedCaseInsensitiveCompare:)];
}
